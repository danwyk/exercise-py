...
# create purse with 1000 golad pieces and cost
purse = 1000
cost = 0

# create a temp_str as the combination of keys and values
result_str = ''

# print all inventory before purchase
inventory = {}
for shop in (freelancers, antiques, pet_shop):
    inventory.update(shop)
del inventory["name"]
# inventory.pop("name")
...
for shop in (freelancers,antiques,pet_shop) :
    ...
    buy_item = input(f"Welcome to {shop['name'].title()}! what do you want to buy:\n     {shop} \nYour choice or quit: ").lower()
    ...
    if buy_item == "quit":
        ...
        print("***** Invalid item, jump to the next shop *****")
        print()
        continue
    else:
        #update the cart
        ...
        result_str = f"{result_str} {buy_item}:{str(cart[buy_item])}"
        cost += cart[buy_item]
...
# print all inventory after purchase
inventory_after = {}
for shop in (freelancers, antiques, pet_shop):
    inventory_after.update(shop)
inventory_after.pop("name")
print(f"Inventory After:\n{sorted(inventory_after.items())}\n\n")

if len(cart) != 0:
    # cost = sum(cart.values())
    result_str = ", ".join(result_str.split())
    print(f'You Purchased "{result_str}".\nThe total cost is {cost}.\nYou have {purse - cost} left in your purse!')
else:
     print("Exit program")